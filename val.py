...
def run(data,
    ...
    if not training:
        ...
        task = task if task in ('all', 'val', 'test') else 'val'  # path to all/val/test images
...
def parse_opt():
    ...
    parser.add_argument('--data', type=str, default=ROOT / 'data/all/tree_overlap_50_patch_640/dataset.yaml', help='dataset.yaml path')
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'runs/ssod/yolov8s_tree/all_tree_overlap_50_patch_640/100%-0.2-0.8-pavilion/weights/best.pt', help='model.pt path(s)')
    parser.add_argument('--batch-size', type=int, default=32, help='batch size')
    parser.add_argument('--imgsz', '--img', '--img-size', type=int, default=640, help='inference size (pixels)')
    parser.add_argument('--conf-thres', type=float, default=0.001, help='confidence threshold')
    parser.add_argument('--iou-thres', type=float, default=0.5, help='NMS IoU threshold')
    parser.add_argument('--task', default='val', help='all, val, test, speed or study')
    ...
    opt = parser.parse_args(['--task=test'])
    #opt = parser.parse_args(['--val-ssod','--task=test'])
    #opt = parser.parse_args(['--save-conf','--save-txt','--val-ssod'])
    if opt.cfg == '':
