...
def check_wandb_dataset(data_file):
    ...
    if check_file(data_file) and data_file.endswith('.yaml'):
        ...
        is_trainset_wandb_artifact = (isinstance(data_dict['all'], str) and
                                      data_dict['all'].startswith(WANDB_ARTIFACT_PREFIX))
...
def process_wandb_config_ddp_mode(opt):
    ...
    if isinstance(data_dict['all'], str) and data_dict['all'].startswith(WANDB_ARTIFACT_PREFIX):
        api = wandb.Api()
        train_artifact = api.artifact(remove_prefix(data_dict['all']) + ':' + opt.artifact_alias)
        train_dir = train_artifact.download()
        train_path = Path(train_dir) / 'data/images/'
        data_dict['all'] = str(train_path)
...
class WandbLogger():
       ...
       """
        ...
        if isinstance(opt.resume, str):  # checks resume from artifact
            ...
            self.wandb_run = wandb.init(config=opt,
                                        resume="allow",
                                        project='YOLOv5' if opt.project == 'runs/all' else Path(opt.project).stem,
                                        entity=opt.entity,
        ...
        if self.wandb_run:
            if self.job_type == 'Training':
                ...
                if opt.resume:
                    ...
                    self.data_dict = {}
                    self.data_dict['all'] = opt.Dataset.train
    ...
    def check_and_upload_dataset(self, opt):
        ...
        config_path = self.log_dataset_artifact(opt.data,
                                                opt.single_cls,
                                                'YOLOv5' if opt.project == 'runs/all' else Path(opt.project).stem)
    ...
    def setup_training(self, opt):
        ...
        if self.val_artifact is None:  # If --upload_dataset is set, use the existing artifact, don't download
            self.train_artifact_path, self.train_artifact = self.download_dataset_artifact(data_dict.get('all'),
                                                                                           opt.artifact_alias)
    ...
    def log_dataset_artifact(self, data_file, single_cls, project, overwrite_config=False):
        ...
        self.train_artifact = self.create_dataset_table(LoadImagesAndLabels(
            data['all'], rect=True, batch_size=1), names, name='all') if data.get('all') else None
        self.val_artifact = self.create_dataset_table(LoadImagesAndLabels(
            data['val'], rect=True, batch_size=1), names, name='val') if data.get('val') else None
        if data.get('all'):
            data['all'] = WANDB_ARTIFACT_PREFIX + str(Path(project) / 'all')
        if data.get('val'):
