...
class FairPseudoLabel:
    ...
    def create_pseudo_label_online_with_gt(self, out, target_imgs, M_s, target_imgs_ori, gt=None, RANK=-2):
        ...
        if target_shape[0] > 0 and len(target_out_targets_perspective) > 0 :
           ...
           if self.debug:
              if RANK in [-1 ,0]:
                draw_image = plot_images_ssod(copy.deepcopy(target_imgs), target_out_targets_perspective, fname='D:/pythonProject/efficientteacher/pseudo_label.jpg', names=self.names)
                draw_image = plot_images(copy.deepcopy(target_imgs), gt, fname='D:/pythonProject/efficientteacher/gt.jpg', names=self.names)
                    # raise 0
