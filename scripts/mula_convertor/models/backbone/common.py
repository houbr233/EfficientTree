...
class Bottleneck(nn.Module):
    # Standard bottleneck
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3,3), e=0.5, act=True):  # ch_in, ch_out, shortcut, groups, expansion, act
        super().__init__()
...
class C3(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5, act=True,k=(1,3)):  # ch_in, ch_out, number, shortcut, groups, expansion, act
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        if act == 'relu_silu':
            act = 'relu'
        ...
        self.m = nn.Sequential(*[Bottleneck(c_, c_, shortcut, g, k, e=1.0, act=act) for _ in range(n)])
...
class C2f(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5, act=True,k=(3,3)):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
        ...
        self.m = nn.ModuleList(Bottleneck(self.c, self.c, shortcut, g, k, e=1.0, act=act) for _ in range(n))
...
class PreConv(nn.Module):
    def __init__(self, c1, c2, g=1, e=0.5, with_aug=True, act=True):
        ...
        self.with_aug = with_aug
        if self.with_aug:
            self.cv0 = Conv(c1, c_, 3, 1, None, g, act)
            self.cv1 = Conv(c_, c2, 3, 2, None, g, act)
        else:
            self.cv1 = Conv(c1, c2, 3, 2, None, g, act)
