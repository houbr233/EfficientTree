...
class YoloV8Detect(nn.Module):
    ...
    def initialize_biases(self):
        ...
        self.proj_conv.weight = nn.Parameter(self.proj.view([1, self.reg_max, 1, 1]).clone().detach(),requires_grad=False)
    ...
    def forward(self, x):
        if self.training:
            ...
            return (torch.cat([pred_bboxes,torch.ones((b, pred_bboxes.shape[1], 1), device=pred_bboxes.device, dtype=pred_bboxes.dtype),cls_list], axis=-1), feature)
