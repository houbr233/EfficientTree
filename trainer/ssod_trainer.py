    ...
    def build_model(self, cfg, device):
        ...
        if pretrained:
            ...
            if self.ema and ckpt.get('ema'):
                ...
                model_ema_state_dict = self.ema.ema.state_dict()
                filtered_ema_state_dict = {k: v for k, v in ema_state_dict.items() if
                                           k in model_ema_state_dict and v.size() == model_ema_state_dict[k].size()}
                model_ema_state_dict.update(filtered_ema_state_dict)
                self.ema.ema.load_state_dict(model_ema_state_dict, strict=False)
