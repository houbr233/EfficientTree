...
class YOLOAnchorAssigner(nn.Module):
    ...
    def build_targets(self, p, targets):
        #p: 网络训练阶段输出，[torch.tensor(n, c, h1, w1), torch.tensor(n, c, h2, w2), (n, c, h3, w3)
        #targets: 数据标签，(m, 6)-(img_id、cls_id、x、y、w、h)，虽然batch = n,但是target>=n，因为单张图片可能存在多个目标
        targets = targets[:,:6]
        #na: 每个尺度建议的框的数量 ，nt:当前batch的目标框数量
        na, nt = self.na, targets.shape[0]
        #tcls：类别，tbox：bbox，indices：图片索引、当前层anchor索引、grid下标
        tcls, tbox, indices, anch = [], [], [], []
        #gain: 标签中归一化后的xywh映射到特征图上的比例，包括6个原始标签数据+建议框id，（m，7）
        gain = torch.ones(7, device=targets.device)
        # ai（na，m），targets 从(m, 6) -> (na,m,7)
        ai = torch.arange(na, device=targets.device).float().view(na, 1).repeat(1, nt)  # same as .repeat_interleave(nt)
        targets = torch.cat((targets.repeat(na, 1, 1), ai[:, :, None]), 2)  # append anchor indices
        # 预定义的偏移量
        g = 0.5
        ...
        for i in range(self.nl):
            ...
            anchors = self.anchors[i]
            # p[i]：[n, c, h, w] -> gain:[1, 1, w, h, w, h, 1]
            gain[2:6] = torch.tensor(p[i].shape)[[3, 2, 3, 2]]

            # 将归一化后的标签映射到特征图上
            t = targets * gain
            if nt:
                # r：anchor与gt 的宽高比值（wh ratio），若大于anchor_t则被舍弃
                r = t[:, :, 4:6] / anchors[:, None]
                j = torch.max(r, 1. / r).max(2)[0] < self.anchor_t
                t = t[j]  # filter

                # 扩增正样本
                gxy = t[:, 2:4]  # 获取target中心点坐标
                gxi = gain[[2, 3]] - gxy  # 反转坐标  使用特征图尺寸-gxy
                j, k = ((gxy % 1. < g) & (gxy > 1.)).T    # j: x%1<0.5 & x>1  k:y%1<0.5 & y>1
                l, m = ((gxi % 1. < g) & (gxi > 1.)).T    # l: x%1<0.5 & x>1  m:y%1<0.5 & y>1
                # yolov5不仅使用目标中心所在的网格预测目标，还使用距目标中心点最近的两个网格
                j  = torch.stack((torch.ones_like(j), j, k, l, m))
                #将t复制5次，用j过滤，1：保留过滤后的gtbox、2：左面gtbox、3：下面gtbox、4：右面gtbox、5：上面gtbox
                t = t.repeat((5, 1, 1))[j]
                # 生成偏移矩阵
                offsets = (torch.zeros_like(gxy)[None] + off[:, None])[j]
            else:
            ...
            # b:img_id、c:cls_id、gxy：grid xy、gwh：grid wh
            b, c = t[:, :2].long().T
            gxy = t[:, 2:4]
            gwh = t[:, 4:6]
            # 针对j、k、l、m分别对t做offset，即偏左上角的点补充左、上的样本，偏左下角的点补充左、下的样本。。。
            gij = (gxy - offsets).long()
            #在原来gt附近补充正样本,gij就是正样本的索引
            gi, gj = gij.T  # grid xy indices
            ...
            indices.append((b, a, gj.int().clamp(0, gain[3] - 1), gi.int().clamp(0, gain[2] - 1)))
            tbox.append(torch.cat((gxy - gij, gwh), 1))  # box
            anch.append(anchors[a])  # anchors
            tcls.append(c)  # class
    ...
    def build_uc_targets_aug(self, p, targets):
        ...
        for i in range(self.nl):
            ...
            score = t[:,6].T
            #indices.append((b, a, gj.clamp_(0, gain[3] - 1), gi.clamp_(0, gain[2] - 1)))  # image, anchor, grid indices
            indices.append((b, a, gj.int().clamp(0, gain[3] - 1), gi.int().clamp(0, gain[2] - 1)))  # image, anchor, grid indices
            tbox.append(torch.cat((gxy - gij, gwh), 1))  # box
            anch.append(anchors[a])  # anchors
            tcls.append(c)  # class
            tscore.append(score)
