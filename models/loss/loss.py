...
from models.assigner.yolo_anchor_assigner import YOLOAnchorAssigner
import numpy as np
...
class ComputeLoss:
    ...
    def __init__(self, model, cfg):
        ...
        self.nl = det.nl
        self.nc = 1 if cfg.single_cls else cfg.Dataset.nc
        self.box_w = cfg.Loss.box*3.0/self.nl
        self.obj_w = cfg.Loss.obj
        self.cls_w = cfg.Loss.cls*self.nc / 80. * 3. / self.nl
        ...
        self.assigner = YOLOAnchorAssigner(det.na, self.nl, det.anchors, self.anchor_t, det.stride,
            self.nc, self.num_keypoints, single_targets=self.single_targets, ota=False)
        self.ota_assigner = YOLOAnchorAssigner(det.na,self.nl, det.anchors, self.anchor_t, det.stride,
    ...
    def default_loss(self, p, targets):
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            b, a, gj, gi = indices[i]  # 正样本 image, anchor, gridy, gridx
            tobj = torch.zeros_like(pi[..., 0], device=device)  # target obj

            n = b.shape[0]  # number of targets
            if n:
                #ps = pi[b, a, gj, gi]  # prediction subset corresponding to targets
                ps = pi[b, a, gj.long(), gi.long()]
                # 边框损失 Regression
                pxy = ps[:, :2].sigmoid() * 2. - 0.5
                pwh = (ps[:, 2:4].sigmoid() * 2) ** 2 * anchors[i]
                pbox = torch.cat((pxy, pwh), 1)  # predicted box
                iou = bbox_iou(pbox.T, tbox[i], x1y1x2y2=False, CIoU=True)  # iou(prediction, target)
                #iou = bbox_iou(pbox.T, tbox[i],xywh=False, CIoU=True)
                ...
                tobj[b, a, gj.long(), gi.long()] = (1.0 - self.gr) + self.gr * score_iou  # iou ratio
